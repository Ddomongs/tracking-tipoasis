...
> [!NOTE]
> **CJ대한통운, 우체국택배, 한진택배, 롯데택배, 로젠택배**를 지원합니다.
> 자동판별 결과가 겹치거나 조회 서비스가 지연되면 사용자가 택배사를 직접 선택하고 공식 조회 페이지로 이동할 수 있습니다.

| 항목 | 내용 |
|------|------|
| **조회 방식** | 각 택배사 공식 배송조회 응답을 서버사이드에서 조회·정규화 |
| **자동판별** | 운송장 길이 기반 후보 조회 후 단일 일치만 자동 선택 |
| **파라미터** | 운송장번호, 택배사 코드(`AUTO`, `CJ`, `EPOST`, `HANJIN`, `LOTTE`, `LOGEN`) |
| **응답형식** | JSON/HTML → 공통 배송 이벤트로 정규화 |
| **지원 택배사** | CJ대한통운, 우체국택배, 한진택배, 롯데택배, 로젠택배 |
| **별도 API 키** | ❌ 불필요 (공개 조회 페이지) |
| **제한사항** | 과도한 요청 시 차단 가능 → 캐싱(15분)으로 대응 |

**조회 페이지:** 각 택배사의 공식 배송조회 페이지를 사용하며, 조회 지연 시 해당 공식 페이지 링크를 제공합니다.
